...
SAVE_FIG_PATH = "./out/dijkstra_path100.png"
...
def get_reasonable_source_target(G, percentile=0.8):
    source = 0
    pos = nx.get_node_attributes(G, "pos")
...
if __name__ == "__main__":
    ...
    with open("./out/source_target.txt") as f:
        source, target = map(int, f.read().split(","))


    print(f"Running Dijkstra from {source} → {target}")
    ...
    SAVE_RESULT_PATH = "./out/dijkstra_paths100.pkl"
    print("\nVisualizing path...")
    ...
    print(f"\nSaved Dijkstra result to {SAVE_RESULT_PATH}")
